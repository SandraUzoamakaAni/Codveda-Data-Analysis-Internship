...
df["t"] = (df["Dates"] - START_DATE).dt.days / 30.44  # months since start

# ---------------------------------------------------------------
# 2. Feature engineering: trend + annual seasonal cycle
# ---------------------------------------------------------------
...
# ---------------------------------------------------------------
# 3. The function required by the task
# ---------------------------------------------------------------
def estimate_price(date_input):
    """
    Return an estimated natural gas purchase price for any given date.
    Works for dates within the historical range (interpolation) and up to
    ~1 year past the last known data point (extrapolation).
    """
